...
function getBattleStatBd(what) {
	...
	if (game.global.challengeActive == "Daily"){
		var mult = 0;
		if (typeof game.global.dailyChallenge.weakness !== 'undefined' && what == 'New'){
		...
		if (typeof game.global.dailyChallenge.oddTrimpNerf !== 'undefined' && what == 'New' && (game.global.world % 2 == 1)){
		...
		if (typeof game.global.dailyChallenge.evenTrimpBuff !== 'undefined' && what == 'New' && (game.global.world % 2 == 0)){
		...
		if (typeof game.global.dailyChallenge.rampage !== 'undefined' && what == 'New'){
		...
		if (typeof game.global.dailyChallenge.pressure !== 'undefined' && what == 'New'){
	...
	if (what != "block" && game.goldenUpgrades.Battle.currentBonus > 0){
		...
		currentCalc *= 1 + amt;
		textString += "<tr><td class='bdTitle'>Golden Battle</td><td></td><td></td><td>+ " + prettify(amt * 100) + "%</td><td class='bdNumberSm'>" + prettify(currentCalc) + "</td>" + ((what == 'New') ? getFluctuation(currentCalc, minFluct, maxFluct) : "") + "</tr>";
	}
	if (what != "block" && game.talents.voidPower.purchased && game.global.voidBuff){
		...
		textString += "<tr><td class='bdTitle'>Void Power (Mastery)</td><td></td><td>" + ((game.talents.voidPower2.purchased) ? 2 : 1) + "</td><td>+ " + amt + "%</td><td class='bdNumberSm'>" + prettify(currentCalc) + "</td>" + ((what == 'New') ? getFluctuation(currentCalc, minFluct, maxFluct) : "") + "</tr>";
	}
	//Magma
	if (mutations.Magma.active() && (what == 'New' || what == 'New')){
		mult = mutations.Magma.getTrimpDecay();
		var lvls = game.global.world - mutations.Magma.start() + 1;
		currentCalc *= mult;
		var display = (mult > 0.0001) ? mult.toFixed(4) : mult.toExponential(3);
		textString += "<tr style='color: red'><td class='bdTitle'>Overheating (Magma)</td><td>x 0.8</td><td>" + lvls + "</td><td class='bdPercent'>x " + display + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td>" + ((what == 'New') ? getFluctuation(currentCalc, minFluct, maxFluct) : "") + "</tr>";
	}
	if (game.global.totalSquaredReward > 0 && (what == 'New' || what == 'New')){
		amt = game.global.totalSquaredReward;
		currentCalc *= (1 + (amt / 100));
		textString += "<tr><td class='bdTitle'>Challenge² Rewards</td><td></td><td></td><td>+ " + amt + "%</td><td class='bdNumberSm'>" + prettify(currentCalc) + "</td>" + ((what == 'New') ? getFluctuation(currentCalc, minFluct, maxFluct) : "") + "</tr>"
	}
	//Ice
	if (what == 'New' && getEmpowerment() == "Ice"){
...
function getLootBd(what) {
	...
	if (game.global.totalSquaredReward > 0 && what == 'New'){
	...
	if (game.global.runningChallengeSquared && what == 'New'){
		currentCalc = 0;
		textString += "<tr class='colorSquared'><td class='bdTitle'>Challenge²</td><td></td><td></td><td>0%</td><td>" + prettify(currentCalc) + "</td></tr>";
	}
	if (what == 'New' && !game.global.voidBuff && (game.global.world >= mutations.Corruption.start())){
	...
	else if (what == 'New' && game.global.voidBuff && game.global.world >= mutations.Corruption.start(true)) {
		//5.71m
...
function filterTabs (what) {
	...
	for (var tab in tabs){
		tab = tabs[tab];
		document.getElementById(tab + "Container").style.display = ((what == 'New' && tab != "talents" && tab != "nature") || tab == what) ? "block" : "none";
	}
	if (what == 'New') displayTalents();
	if (what == 'New') displayNature();
...
function numTab(what, p, fromRestore) {
	var num = 0;
	if (what == 'New' && game.global.buyAmt == "Max") tooltip('Max', null, 'update');
	if (what == 5){
		unlockTooltip();
		tooltip('hide');
...
function unlockUpgrade(what, displayOnly) {
	...
	if (upgrade.prestiges){
		var resName = (what == 'New') ? "wood" : "metal";
...
function updateButtonColor(what, canAfford, isJob) {
	...
	if(canAfford){
		if
			(what == 'New' && (ctrlPressed || game.options.menu.ctrlGigas.enabled)) swapClass("thingColor", "thingColorCtrl", elem);
		else
