...
function toggleGeneticistassist(updateOnly){
	...
	if (!updateOnly){
		...
		if (currentStep > 0){
			...
			GATimeout = setTimeout(function(){ indicatorElem.innerHTML = ' (1)' }, 1000 * TIMEOUT_MULTI);
...
function costUpdatesTimeout() {
	...
	setTimeout(costUpdatesTimeout, 250 * TIMEOUT_MULTI);
}
...
function buyAutoStructures(){
	setTimeout(buyAutoStructures, 350 * TIMEOUT_MULTI);
	if (game.options.menu.pauseGame.enabled)
		return;
	var setting = game.global.autoStructureSetting;
...
function gameTimeout() {
	...
	if ((now - game.global.start - game.global.time) > 3600000){
		...
		setTimeout(gameTimeout, (1000 * TIMEOUT_MULTI / game.settings.speed));
		return;
	}
	game.global.lastOnline = now;
...
function updatePortalTimer(justGetTime) {
	...
	setTimeout(updatePortalTimer, 1000 * TIMEOUT_MULTI);
}
